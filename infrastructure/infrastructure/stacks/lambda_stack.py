...
class LambdaStack(Stack):
    ...
    def __init__(
        ...
        construct_id: str,
        database_stack,
        event_stack=None,  # Optional, for EventBridge orchestration
        **kwargs
    ) -> None:
        ...
        # Get resources from EventStack if provided
        if event_stack:
        ...
        # Note: You need to build this first with: cd backend/shared && npm run build
        ...
        # Common env variables for all Lambdas
        common_env = {
        ...
        # Add EVENT_BUS_NAME only if EventStack exists
        if event_bus:
        ...
        # API Functions need read/write access to tables and publish to EventBridge
        ...
        # Notification sender needs SES permissions
        self.send_notification_fn.add_to_role_policy(
            iam.PolicyStatement(
                actions=["ses:SendEmail", "ses:SendRawEmail"],
                resources=["*"],  # In production, scope this down to specific email addresses
            )
        )
        ...
        # Grant send notification access to DynamoDB
